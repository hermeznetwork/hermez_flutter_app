...
import 'package:hermez/components/form/email_input.dart';
...
class BitrefillFormArguments {
  ...
  BitrefillFormArguments(
    ...
    this.token,
      ...
      /*this.txLevel, this.transactionType,
      {this.account,
      this.token,
      this.amount,
      this.addressTo,
...
class BitrefillFormPage extends StatefulWidget {
  BitrefillFormPage({Key key, this.arguments}) : super(key: key);

  final BitrefillFormArguments arguments;

  @override
  _BitrefillFormPageState createState() => _BitrefillFormPageState();
}

class _BitrefillFormPageState extends State<BitrefillFormPage>
/*with AfterLayoutMixin<TransactionAmountPage>*/ {
    ...
    return Column(
      children: <Widget>[
        Container(
          decoration: BoxDecoration(
            border: Border.all(
              color: emailIsValid || needRefresh
                  ? HermezColors.blueyGreyThree
                  : HermezColors.redError,
              width: 2,
            ),
                ...
                Expanded(
                  child: EmailInput(
                    ...
                    onChanged: (value) async {
                      bool valid = await isEmailValid(value);
                      setState(() {
                        emailIsValid = valid;
                        if (emailIsValid) {
                          //amountController.clear();
                            ...
                            child: TextButton(
                              ...
                              onPressed: () {
                                getClipBoardData().then((String result) async {
                                  bool valid = await isEmailValid(result);
                                  setState(() {
  ...
  Future<bool> isEmailValid(String email) async {
    return email.isEmpty || EmailUtils.isValidEmailAddress(email);
  }
